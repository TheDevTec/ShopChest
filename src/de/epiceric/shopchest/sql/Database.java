...
public abstract class Database {
            ...
            public void run() {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        Shop shopAdding = new Shop(id, plugin, vendor, product, location, buyPrice, sellPrice, shopType);
                        shops.add(shopAdding);
    ...
    public void findItem(final Player player, final String search, final Callback callback) {
        new BukkitRunnable() {
            @Override
            public void run() {
                PreparedStatement ps = null;
                ...
                String searchterm = ChatColor.stripColor(search).toLowerCase();
                ...
                try {
                    ps = connection.prepareStatement("SELECT * FROM shops");
                    rs = ps.executeQuery();
                    List<Shop> foundList = new ArrayList<Shop>();
                    while (rs.next()) {
                        ItemStack product = Utils.decode(rs.getString("product"));
                        String itemName = product.getType().name().replace("_", " ");
                        if (product.getItemMeta() != null)
                        {
                            if (product.getItemMeta().hasDisplayName())
                            {
                                itemName = product.getItemMeta().getDisplayName();
                            }
                        }
                        if (ChatColor.stripColor(itemName).toLowerCase().contains(searchterm)) {
                            int id = rs.getInt("id");
                            ...
                            if (world == null) {
                                WorldNotFoundException ex = new WorldNotFoundException("Could not find world with name \"" + worldName + "\"");
                                if (!notFoundWorlds.contains(worldName)) plugin.getLogger().warning(ex.getMessage());
                                notFoundWorlds.add(worldName);
                                plugin.debug("Failed to get shop (#" + id + ")");
                                plugin.debug(ex);
                                continue;
                            ...
                            ShopType shopType = ShopType.valueOf(rs.getString("shoptype"));
                            Shop shopAdding = new Shop(id, plugin, vendor, product, location, buyPrice, sellPrice, shopType);
                            foundList.add(shopAdding);



                        }
                    }
                    if (callback != null) callback.callSyncResult(foundList);
                } catch (SQLException ex) {
                    if (callback != null) callback.callSyncError(ex);
                    plugin.getLogger().severe("Failed to access database");
