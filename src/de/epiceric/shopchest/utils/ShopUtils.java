...
public class ShopUtils {
                    ...
                    if (shop.getLocation().distanceSquared(player.getEyeLocation()) <= itemDistSqr) {
                        ...
                        if (Utils.getAmount(shop.getInventoryHolder().getInventory(), shop.getProduct()) < shop.getProduct().getAmount() && shop.getBuyPrice() != 0  && shop.getShopType() == Shop.ShopType.NORMAL) {
                            if (shop.getNoItem() != null) {
                                shop.getNoItem().setVisible(player, true);
                            }
                        }
                        else
                        {
                            if (shop.getNoItem() != null) {
                                shop.getNoItem().setVisible(player,false);
                            }
                        }
                    } else {
    ...
    public void updateShop(Shop shop, Player player) {
        if (!shop.isChunkLoaded()) return; // this was if the chunk was loaded exit??? why
        shop.checkShopStatus();
        ...
        if (player.getLocation().getWorld().getName().equals(shop.getLocation().getWorld().getName())) {
            ...
            if (distSqr <= itemDistSqr) {
                ...
                if (shop.getNoItem() != null) shop.getNoItem().setVisible(player, false);
            }
        }
    }
}
